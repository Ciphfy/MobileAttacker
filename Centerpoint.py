...
class Centerpoint:
    ...
    def brute_force_centerpoint(self, point_set):
        ...
        while len(point_set) > 10:
            ...
            if len(p_corner) == 1:
                return p_corner #delete enough points and only 1 point remained, then this one is the centerpoint(The best but specially rare case)
            for p in p_corner:
                point_set.remove(p)
            if len(p_corner) == 4:
        ...
        empty_inter = True
        iter_set = list(itertools.combinations(point_set, n - f)) #list all the points set with cardinality of N-F
        random.shuffle(iter_set) #randomly change the sequence
        for point_set in iter_set:
            if empty_inter:
                ...
                polygon = Polygon([p for p in point_set]).convex_hull #directly calculating the intersection of all sets listed above
