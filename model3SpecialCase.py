...
for k in range(0,50): #set iteration times
    ...
    for a in agent:
        if a.no == attack_last_obj:
            ...
            pos_t[a.no] = a.pos
        '''differences to distinguish model2 with model1'''
    
    for a in agent:
        a.find_update_set_with_flag(pos_t, attack_last_obj)#differences to distinguish model3 with model2